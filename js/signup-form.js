...
(function () {
  ...
  terms.addEventListener('change', () => SmartCareFormHelpers.showError(terms, false));
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (fullName.value.trim().length < 2) {
      SmartCareFormHelpers.showError(fullName, true, 'Enter your full name (at least 2 characters).'); valid = false;
    }
    ...
    if (!SmartCareValidators.isValidPhone(phone.value.trim())) {
      SmartCareFormHelpers.showError(phone, true, 'Enter a valid phone number (11 digits).'); valid = false;
    }

    const checks = SmartCarePasswordStrength.evaluate(password.value);
    if (!SmartCareValidators.passwordRulesPassed(checks)) {
      SmartCareFormHelpers.showError(password, true, "Password doesn't meet all the requirements above."); valid = false;
    }

    const matchOk = confirmPassword.value !== '' && confirmPassword.value === password.value;
    if (!matchOk) {
      const msg = confirmPassword.value === '' ? 'Re-enter your password to confirm it.' : "Passwords don't match.";
      SmartCareFormHelpers.showError(confirmPassword, true, msg); valid = false;
    }

    if (!terms.checked) {
      SmartCareFormHelpers.showError(terms, true, 'Please accept the Terms and Privacy Policy to continue.'); valid = false;
    }
    ...
    fetch('../php/signup.php', { method: 'POST', body })
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          showToast({ type: 'error', title: 'Sign up failed', body: data.message });
        }
      })
      .catch(() => {
